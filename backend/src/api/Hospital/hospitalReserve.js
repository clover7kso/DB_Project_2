        ...
        if (result.length === 0) {
          ...
          connection.query(
            ...
            (error1, data1) => {
              ...
              for (
                var i = 0;
                i < result1[0].lunchEnt1 - result1[0].lunchStt1;
                i++
              ) {
                //점심시간 처리
          ...
          for (var i = 0; i < result.length; i++) {
            if (result[i].reserve_date === Reserve.date) {
              //일치하는 날짜일경우 예약되어 있는 시간개수만큼 여석 줄여줌
              Reserve[result[i].reserve_hour]--;
            }
          }
          var lunch_start = result[0].lunchStt;
          for (var i = 0; i < result[0].lunchEnt - result[0].lunchStt; i++) {
            //점심시간 처리
            Reserve[lunch_start++] = 0;
          }
          //console.log(Reserve);
